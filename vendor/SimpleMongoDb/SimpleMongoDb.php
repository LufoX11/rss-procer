 ...
 * Example usage:
 *   $mongo = new Mongo();
 *   SimpleMongoDb::addConnection($mongo, 'lost');
 *
 ...
 *   ));
 *   foreach (SimpleMongoDb::find('people', array('goodguy' => true), array('sort' => array('name' => 1))) as $p) {
 *     echo $p['name'] " is a good guy!\n";
 *   }
 *   $ben = SimpleMongoDb::findOne('people', array('name' => 'Ben'));
 *   $locke = SimpleMongoDb::findOne('people', array('name' => 'Locke'));
 *   $ben['enemy'] = SimpleMongoDb::createRef('people', $locke);
 *   $ben['goodguy'] = null;
 *   SimpleMongoDb::save('people', $ben);
 *
 * See the SimpleMongoDbo.php class for how you could do the same thing with data objects.
 *
...
class SimpleMongoDb {
    ...
    static function getConnectionInfo($collection = null, $read_only = false) {
        ...
        if (!$info) {
            if (isset(self::$connections[$collection])) {
                ...
                throw new Exception('No connection configuration, call SimpleMongoDb::addConnection()');
            }
        }
    ...
    /**
     * Returns a MongoId from a string, MongoId, array, or SimpleMongoDbo object
     *
     * @param mixed $obj
     * @return MongoId
     **/
