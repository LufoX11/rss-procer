 ...
 *    - include() or require() the SimpleMongoDb.php file, and this file
 *    - call SimpleMongoDb::addConnection()
 *    - create any number of data object classes that extend SimpleMongoDbo
 *    - call SimpleMongoDbo::addClass(<class name>, <collection name>) for each class
 *
 * Example usage (this code does basically the same thing as the sample code in SimpleMongoDb.php)
 *   $mongo = new Mongo();
 *   define('MONGODB_NAME', 'lost');
 *   class LostPerson extends SimpleMongoDbo {
 *     public function rollCall() {
 *       echo "$this->name is a " . ($this->goodguy ? 'good' : 'bad') . " guy!\n";
 *     }
 *   }
 *   SimpleMongoDbo::addClass('LostPerson', 'people');
 *
 *   SimpleMongoDb::drop('people');
 *   SimpleMongoDb::batchInsert('people', array(
 ...
 *   ));
 *   foreach (SimpleMongoDbo::find('LostPerson', array('goodguy' => true), array('sort' => array('name' => 1))) as $p) {
 *     $p->rollCall();
 *   }
 *   $ben = SimpleMongoDbo::findOne('LostPerson', array('name' => 'Ben'));
 *   $locke = SimpleMongoDbo::findOne('LostPerson', array('name' => 'Locke'));
 *   $ben->enemy = SimpleMongoDbo::toRef($locke);
 *   $ben->goodguy = null;
 *   SimpleMongoDbo::save($ben);
 *
...
class SimpleMongoDbo {
    ...
    public function reload()
    {
        if (isset($this->_id)) {
            ...
            $this->_data = SimpleMongoDb::findOne($collection, $this->_id);
        }
    }
...
// Static functions
    ...
    static function getClass($collection) {
        if (!isset($GLOBALS['MONGODB_CLASSES'][$collection])) {
            throw new Exception("SimpleMongoDbo::getClass cannot find $collection class");
        }
    ...
    static function getCollection($class) {
        if (!isset($GLOBALS['MONGODB_COLLECTIONS'][$class])) {
            throw new Exception("SimpleMongoDbo::getCollection cannot find $class collection");
        }
     ...
     * @param array $query
     * @param array $options
     * @return SimpleMongoDboiterator
     **/
    static function find($class, $query = array(), $options = array()) {
        $collection = self::getCollection($class);
        $result = SimpleMongoDb::find($collection, $query, $options);
        return new SimpleMongoDboiterator($class, $result);
    }

    /**
     * Just like SimpleMongoDb::finda() but will return an array of objects
     *
     * @param string $class
     * @param array $query
     * @param array $options
     ...
     * @param string $class
     * @param mixed $id
     * @return SimpleMongoDbo
     **/
    static function findOne($class, $id = array()) {
        $collection = self::getCollection($class);
        $data = SimpleMongoDb::findOne($collection, $id);
    ...
    static function count($class, $query = array()) {
        global $mongo;
        $collection = self::getCollection($class);
        $count = SimpleMongoDb::count($collection, $query);
    ...
    static function save($object) {
        ...
        $collection = self::getCollection($class);
        $object->presave();
        return SimpleMongoDb::save($collection, $object->_data);
    }
    ...
    static function remove($object) {
        ...
        $collection = self::getCollection($class);
        $object->preremove();
        return SimpleMongoDb::remove($collection, array('_id' => $object->_id));
    }
    ...
    static function getRef($dbref) {
        ...
        $data = SimpleMongoDb::getRef($dbref);
        return $data ? new $class($data) : null;
    }
    ...
    static function getRefs($collection, array $ids) {
        ...
        $data = SimpleMongoDb::find($collection, array(
            '_id' => array('$in' => $ids)
        ));
        $objects = array();
    ...
    static function expandRefs($value, $keep_nulls = false) {
        if (SimpleMongoDb::isRef($value)) {
            return self::getRef($value);
        }
        elseif (is_array($value)) {
            // TODO: Temporay fix, remove this later
            if (empty($value) || ($value[0] instanceof SimpleMongoDbo)) 
                return $value;
    ...
    static function toRef($value) {
        if ($value instanceof SimpleMongoDbo) {
...
/**
* Helper iterator class for SimpleMongoDbo::find(), implements the Iterator interface so you can
* foreach your way through the returned result and not worry about the details.
**/
class SimpleMongoDboiterator implements Iterator, Countable {
    private $class;
    private $resultset;
