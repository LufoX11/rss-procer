  ...
  **/
	...
	class WideImage
	{
        ...
        public static $supportedFormats = array(
            'gif', 'png', 'jpg', 'gd', 'gd2', 'wbmp', 'xbmp', 'xpm', 'bmp', 'tga', 'jpeg', 'swf', 'tiff');
        ...
        /**
         * Checks if the file has a valid format by its extension.
         *
         * @param $file string File to check.
         * @return boolean TRUE if valid; FALSE otherwise.
         */
        public static function isValidFormat($file)
        {
            $res = false;

            // Remove query string
            if (($qpos = strpos($file, '?')) !== false) {
                $file = mb_substr($file, 0, $qpos);
            }

            $fileInfo = pathinfo($file);
            if (isset($fileInfo['extension'])) {
                $res = in_array(strtolower($fileInfo['extension']), self::$supportedFormats);
            }
